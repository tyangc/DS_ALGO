...
There is a matrix before before with nn rows and mm columns. For each element in before before[i][j]before[i][j], we will use the following algorithm to convert it to after [i] [j]after[i][j]. 
Given the afterafter matrix, please restore the original matrix beforebefore.
